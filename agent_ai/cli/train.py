...
def train_command(
    ...
    示例:
        agent train
        agent train --data ./my_data --output ./my_models
        agent train --dry-run  # 仅验证
    """
    verbose = ctx.obj.get("verbose", False)
    debug = ctx.obj.get("debug", False)
    
    click.echo("=" * 50)
    click.echo("Agent AI - 模型训练")
    click.echo("=" * 50)
    ...
    try:
        # 导入训练模块
        from agent_ai.training.trainer import Trainer, TrainerConfig
        from agent_ai.shared.config import AgentConfig
        
        # 加载配置（用于验证）
        click.echo("加载配置...")
        agent_config = AgentConfig.load(str(config_path))
        click.echo("  配置加载成功")
        ...
        if dry_run:
            ...
            from agent_ai.dialogue_understanding.flow import FlowLoader
