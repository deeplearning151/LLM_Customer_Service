...
- agent_ai/ 是我们要安装的包（里面有 __init__.py）
- 所以 setup.py 放在 agent_ai/ 的父目录，即项目根目录
...
2. agent_ai 包会被注册到 Python 环境，可以在任何地方 import
3. agent 命令会被注册到系统，可以在命令行直接使用：
   - agent init    # 初始化项目
   - agent train   # 训练模型
   - agent run     # 启动服务
   - agent inspect # 交互式测试
...
Q: 安装后怎么卸载？
A: pip uninstall agent_ai

Q: 怎么查看是否安装成功？
A: pip show agent_ai  或者  python -c "import agent_ai; print(agent_ai.__version__)"
"""
...
def read_version() -> str:
    """从 agent_ai/__init__.py 读取版本号。
    
    为什么版本号定义在 __init__.py 而不是这里？
    - 这样代码里可以通过 agent_ai.__version__ 获取版本
    - 版本号只需要维护一处，不会出现不一致
    """
    init_path = Path(__file__).parent / "agent_ai" / "__init__.py"
...
setup(
    ...
    # 简短描述（一句话）
    description="智能对话系统 - 基于 LLM 驱动的智能对话架构精简实现",
    ...
    # 
    # include: 只包含这些包（agent_ai 和它的所有子包）
    ...
    # 命令行入口点（CLI）
    # ------------------------------------------------------------------------
    # 安装后会创建 agent 命令，执行时调用 agent_ai.cli 模块的 main 函数
    # 
    # 格式："命令名=模块路径:函数名"
    # 安装后可以直接在终端运行：agent init / agent train / agent run
    entry_points={
        "console_scripts": [
            "agent=agent_ai.cli:main",
        ],
    },
